...
export default function Navbar() {
  ...
  return (
    <header className="navbar premium-navbar modern-blue-navbar">
      ...
      <nav className="desktop-nav premium-desktop-nav">
        {navItems.map(({to,label,icon:Icon}) => <NavLink key={to} to={to} className={({isActive})=>isActive?'nav-link active':'nav-link'}><Icon size={16}/>{label}</NavLink>)}
      ...
      <div className="nav-actions premium-nav-actions">
        ...
        {user ? <button className="nav-user" type="button" onClick={handleLogout}><span className="nav-user-avatar">{user.name?.[0] || 'U'}</span><LogOut size={15}/></button> : <Link className="nav-login" to="/login">Đăng nhập</Link>}
      </div>
    </header>
  );
}
